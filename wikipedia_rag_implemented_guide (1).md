...
```mermaid
graph TD
    A[HuggingFace Wikipedia Dataset<br/>20220301.simple, 100개 문서] --> B[OpenAI text-embedding-3-small<br/>1536차원 벡터]
    B --> C[Pinecone Vector DB<br/>wiki 인덱스]
    C --> D[한국어 질의 지원<br/>벨기에는 어디 있나요?]
    
    D --> E[Belgium 문서 검색 성공<br/>5개 관련 청크 발견]
    E --> F[메타데이터 추출<br/>title, wiki_id, url, chunk_id]
    F --> G[문맥 기반 답변<br/>지리적 위치 및 특징]
...
```mermaid
graph TD
    A[HuggingFace datasets<br/>wikipedia 20220301.simple] --> B[100개 문서 샘플<br/>split=train[:100]]
    B --> C[4개 필드<br/>id, url, title, text]
    C --> D[다양한 주제<br/>April, Alan Turing, Art, Biology 등]
    
    E[RecursiveCharacterTextSplitter<br/>chunk_size=1000, overlap=200] --> F[배치 처리<br/>batch_size=100]
    F --> G[총 600개 청크 생성<br/>메타데이터 포함]
