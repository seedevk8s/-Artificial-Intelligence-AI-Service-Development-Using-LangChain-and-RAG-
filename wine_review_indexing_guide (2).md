...
def recommand_dishs(query):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", """
            Persona: 와인 전문가로서 포도 품종, 지역, 테이스팅 노트, 
            음식 페어링에 대한 광범위한 지식을 보유
            
            Role:
            1. Wine & Food Pairing: 특정 요리와 조화롭게 어울리는 와인 추천
            2. Wine Selection Guidance: 다양한 상황에 맞는 와인 선택 지원
            3. Wine Tasting Expertise: 산미, 탄닌, 단맛, 바디감 기반 와인 식별
            4. Educational Role: 와인 지역, 생산 기법, 스타일에 대한 교육
        """)
    ])
...
#### 테스트 1: 와인 이미지 기반 음식 추천
**입력:**
- 이미지: Jakob Schneider Niedershäuser Felsensteyer Riesling
- 텍스트: "이 와인에 어울리는 요리를 추천해줘"

**출력:**
```
이 Jakob Schneider Niedershäuser Felsensteyer Riesling은 드라이한 스타일로, 
상큼한 산미와 과일 향이 특징입니다. 추천 요리:

1. 해산물 요리: 구운 조개나 새우 요리 (레몬 버터 소스)
2. 닭고기 요리: 허브 양념 구운 닭고기, 레몬 마늘 닭가슴살
3. 아시아 요리: 스파이시한 태국식 커리, 쌀국수
4. 샐러드: 사과나 배가 포함된 신선한 채소 샐러드
```

#### 테스트 2: 음식 이미지 기반 맛 분석
**입력:**
- 이미지: 라자냐 시트 + 버섯 + 브로드빈 요리
- 텍스트: "이 요리의 이름과 맛과 향과 같은 특징을 한 문장으로 설명해줘"

**출력:**
```
이 요리는 부드러운 파스타와 신선한 허브, 고소한 버섯이 조화를 이루며, 
자연의 풍미가 가득한 상큼하고 깊은 맛을 제공합니다.
```
